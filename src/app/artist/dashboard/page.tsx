...
import { ArtworkProps } from "@/components/ui/ArtCard";
                            ...
                            {artworks.map((art) => (
                                <div key={art.id} className="group bg-white rounded-3xl overflow-hidden border border-gray-100 hover:shadow-2xl transition-all duration-300">
                                    <div className="aspect-[4/3] relative overflow-hidden bg-gray-100">
                                        <Image src={art.imageUrl} alt={art.title} fill className="object-cover transition-transform duration-500 group-hover:scale-110" />
                                        ...
                                        <div className="flex justify-between items-start mb-2">
                                            <div>
                                                ...
                                                <p className="text-indigo-600 text-sm font-medium">
                                                    by {typeof art.artist === "object" ? art.artist.name : art.artist || "Unknown Artist"}
                                                </p>
                                            </div>
                                            <span className="text-indigo-600 font-bold">${art.price}</span>
                                        </div>
                                        <p className="text-gray-400 text-sm mb-4">
                                            {art.medium} • Published {art.createdAt ? new Date(art.createdAt).toLocaleDateString() : "N/A"}
                                        </p>
                                    ...
                                    <div className="relative border-2 border-dashed border-gray-200 rounded-[2rem] aspect-square flex flex-col items-center justify-center bg-gray-50 overflow-hidden group hover:border-indigo-400">
                                        {preview ? (
                                            <div className="relative w-full h-full">
                                                ...
                                                <button type="button" onClick={() => { setFile(null); setPreview(null); }} className="absolute top-4 right-4 bg-red-500 text-white p-2 rounded-full text-xs z-10">✕</button>
                                            </div>
                                        ) : (
